...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        success_df = spacex_df[['Launch Site', 'class']].groupby('Launch Site')['class'].sum().reset_index()
        success_df.columns = ['Launch Site', 'count']
        fig = px.pie(data_frame=success_df,
                     values='count', 
                     names='Launch Site',
                     title='Total Successful Launches by Site')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site']==str(entered_site)]
        filtered_success_df = filtered_df['class'].value_counts().reset_index()
        filtered_success_df.columns = ['Success', 'count']
        fig = px.pie(data_frame=filtered_success_df,
                     values='count', 
                     names='Success', 
